function plotMesh2(obj, MNE_ID)
%function plotMesh2(NodeTable, ix, PlotID, elemlist, MNE_ID, varargin)
% Tim Truster
% 11/26/2013
% 
...
% plotMesh2(NodeTable,ix,1,(1:size(ix,1)),[1 0 0 0 0])
%
% Inputs:
%  NodeTable = nodal coordinates [x y]
%  ix = element connectivity [n1 n2 ... nnen]
...
% Default arguments
%
Coordinates = transpose(reshape([obj.nodes.coordinates],2,[]));
disp = transpose(reshape([obj.dofs.v],2,[]));
NodeTable = Coordinates + disp;
numel = obj.ne;
neNodes = obj.elements(1).neNodes;
ix = zeros(numel, neNodes);
for temp = 1:numel
    ix(temp, :) = [obj.elements(temp).eNodes.id];
end
ix = [ix, ones(numel,1)];
PlotID = 1;
elemlist = 1:numel;
...
plotOri = 1; % plot the reference configuration
%
switch 5
    case 1
        error('Must supply NodeTable and ix')
    case 2
        ModelID = 1;
        clfyn = 1;
        subp = 1;
...
for elemp = 1:length(elemlist)
    ...
    PatchPointsOri = Coordinates(ix(elem,1:nel),:);
    PatchPoints = NodeTable(ix(elem,1:nel),:);
    ...
    if nel == 3 || nel == 6
        ...
        if meshyn
            if plotOri
                hold on
                plot([PatchPointsOri(1,1) PatchPointsOri(2,1)],[PatchPointsOri(1,2) PatchPointsOri(2,2)],'r-','LineWidth',1)
                plot([PatchPointsOri(2,1) PatchPointsOri(3,1)],[PatchPointsOri(2,2) PatchPointsOri(3,2)],'r-','LineWidth',1)
                plot([PatchPointsOri(3,1) PatchPointsOri(4,1)],[PatchPointsOri(3,2) PatchPointsOri(4,2)],'r-','LineWidth',1)
                plot([PatchPointsOri(4,1) PatchPointsOri(1,1)],[PatchPointsOri(4,2) PatchPointsOri(1,2)],'r-','LineWidth',1)
                hold off
            end
            if ecolyn
            hold on
            ...
            hold on
            plot([PatchPoints(1,1) PatchPoints(2,1)],[PatchPoints(1,2) PatchPoints(2,2)],'k','LineWidth',2)
            plot([PatchPoints(2,1) PatchPoints(3,1)],[PatchPoints(2,2) PatchPoints(3,2)],'k','LineWidth',2)
            plot([PatchPoints(3,1) PatchPoints(4,1)],[PatchPoints(3,2) PatchPoints(4,2)],'k','LineWidth',2)
            plot([PatchPoints(4,1) PatchPoints(1,1)],[PatchPoints(4,2) PatchPoints(1,2)],'k','LineWidth',2)
            hold off
            end
        end
